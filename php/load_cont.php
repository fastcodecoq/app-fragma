...
$cont = file_get_contents( server_name .  "/themes" . theme_dir . "/pages/". $vista , true); 
$sview = file_get_contents( server_name .  "/themes" . theme_dir ."/pages/sliders.html" , true);
$vars = array();
 ...
 if(preg_match("/^.+[.].+\?/i", $vista)) 
   {
      ...
      if(preg_match("/%print_sliders:.*%|%print_sliders%/i", $sview, $flag)){        
        ...
        $surl = server_name . "/panel/core/php/app.getSliders.php";         
     ...
     switch ($tagName) {
      ...
      if(is_array($sliders))
        ...
        $sview = str_replace("/%print_sliders:.*%|%print_sliders%/i", $slds, $sview);        
     ...
     if(!isset($_COOKIE["dev"]))
       $cont = preg_replace("/%.+%/i", "", $cont);
        ...
        $cont = str_replace("%editable%", "data-role='editable' title='editar' contenteditable", $cont);
        preg_match_all("/%vista:.+%/", $cont, $rs);
     ...
     foreach ($rs as $r) {
        ...
        $cont = preg_replace("/%vista:[a-zA-Z0-9._-]*%/i", "data-role='vista' data-view='$r'", $cont, 1);
        ...
        $b_view = str_replace("%print_logo%", $msg_seo, $b_view); 
         ...
         if(preg_match("/%print_header%/i", $b_view)){
          ...
          $html = file_get_contents(server_name . "/themes" . theme_dir . "/header.php");
         ...
         if(preg_match("/%print_foot%/i", $b_view)){
          ...
          $html = file_get_contents($path . "/themes" . theme_dir  . "/pie.php");
